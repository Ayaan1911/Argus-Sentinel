...
This is the source-of-truth for what fields exist in `argus-intelligence/{services,technologies,vulnerabilities}/*.json`, which are actually required, and — critically — which ones any backend code actually reads versus which are documentation-only. It was written by auditing every one of the 51 entries on file plus every place `backend/app/engines/*.py`, `backend/app/scanners/processor.py`, `backend/app/scanners/httpx.py`, and `backend/app/scanners/nmap.py` read from an intelligence entry, not by reverse-engineering intent from field names.
...
## Fields present in every one of the 51 entries (universal, and required by `schema.json`)
...
These six fields are required on every entry regardless of category. As of the 2026-09 placeholder audit, `schema.json` additionally requires a real `risk_level` or `severity` value on every entry, and requires `vulnerabilities/` entries specifically to carry at least one non-empty `nuclei_tags` or `title_keywords` array — see "What's genuinely optional versus what should always be filled in" at the bottom for why that rule exists.
...
## `technologies/` — one consistent shape (the sparse shape was removed in the 2026-09 audit)

12 files, all using the same full shape as `services/` above (`service`/`technology`, `port`, `protocol`, `risk_level`, `attack_patterns`, `recommended_actions`, `correlation_rules`, `related_findings`, `mitre_attack`, `risk_weight`, etc.), because a `technologies/` entry is matched and consumed by `processor.py` exactly like a `services/` one is (`type == "technology"` reads the same fields as `type in ["port","service"]`, minus `risk_weight` — see below).

This category used to also contain a sparse-shape batch (`django`, `docker` [the technology entry — distinct from the real, port-2375 `services/docker.json`], `iis`, `kubernetes`, `nodejs`, `php`) carrying only `technology`/`type`/`purpose`/`description`/`common_risks`/`mitigations`/`audience_guidance`/`learning_resources`/`version`/`last_updated`/`reviewed_by` — no `risk_level`, no `min_secure_version`/CVE reference, no `attack_patterns`/`recommended_actions`/`correlation_rules`. Because matching is by filename alone, these entries *were* reachable by a real httpx tech-detect hit, but every field a scan's output actually surfaces (`risk_level`-driven display, `recommended_actions`, `attack_patterns`) came back empty — functionally inert. The 2026-09 audit removed all six rather than fabricate a `risk_level`/`min_secure_version`: Django/Kubernetes are platforms whose real risk depends entirely on deployment-specific configuration (`DEBUG` mode, RBAC/API-server exposure) with no single uncontroversial baseline severity; IIS/Node.js/PHP are runtimes/servers broad enough that one `risk_level` would misrepresent most real deployments, and none had a specific, CVE-backed `min_secure_version` floor worth standing behind (unlike `jquery`/`bootstrap`/`apache`/`nginx` below, which cite a specific CVE each). See `notes.md` for the full before/after.

**If you're adding a new technology entry, match this full shape** — it's now the only shape in this category, so there's one clear template to copy.
...
**Optional**: `min_secure_version` — present on 4 of 12 (`apache`, `nginx`, `jquery`, `bootstrap`). Two *separate* consumers exist for this field depending on how the technology is detected:
...
`schema.json` requires the 6 universal fields, a real `risk_level`/`severity`, and (for `vulnerabilities/`) at least one non-empty `nuclei_tags` or `title_keywords` array — the actual common denominator across all 51 existing files now that the six placeholder-shaped `technologies/` entries described above have been removed rather than retrofitted. But "passes the schema" and "is a useful, non-padding entry" are still different bars: a new entry should always include `recommended_actions`, `attack_patterns`, and `risk_weight` too — omitting these doesn't fail validation, but it does mean the entry behaves like those six removed entries did before they were deleted: present in the library, browsable in the UI, but contributing little beyond a description and generic guidance text.
