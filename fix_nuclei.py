...
async def run(target: str) -> list[dict]:
    ...
    command = ["/usr/local/bin/nuclei", "-u", target, "-jsonl", "-silent", "-severity", "low,medium,high,critical"]
    findings = []
    try:
        ...
        if code != 0 or not stdout:
            # Fallback to -json if -jsonl fails
            if "flag provided but not defined: -jsonl" in stderr or "flag provided but not defined" in stderr:
                logger.info(f"nuclei -jsonl failed, falling back to -json")
                ...
                stdout = stdout_bytes.decode('utf-8', errors='replace')
                stderr = stderr_bytes.decode('utf-8', errors='replace')
                logger.info(f"nuclei fallback stdout length: {len(stdout)}")
                if stderr:
                    logger.warning(f"nuclei fallback stderr: {stderr[:500]}")
